...
interface PlayerControlsProps {
  usingServer: boolean; // Define the prop type as a boolean
}

const PlayerControls: React.FC<PlayerControlsProps> = ({usingServer}) => {
  ...
  const clientContext = useContext(GameClientContext)
  ...
  const handleKeyPress = (event: KeyboardEvent) => {
    if (event.key == "w") {
      ...
      if (usingServer) {
        ...
        clientContext?.updateVehicle(clientContext.vehicle.id, "moveForward")
      }      
    }
    if (event.key == "s") {
